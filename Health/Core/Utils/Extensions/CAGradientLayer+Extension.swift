...
/*
 ...
 extension CAGradientLayer {
	 ...
	 private func setupMidnightBlackGradient() {
		 ...
		 if isDarkMode {
			 // 다크 모드: 그라디언트 적용 (위쪽이 더 어둡게)
			 colors = UIColor.GradientColors.midnightBlackDark.reversed().map { $0.cgColor }
			 locations = [0.0, 0.25, 1.0]  // 위→아래: 밝은회색→검은색
		 } else {
			 // 라이트 모드: 흰색 단색
			 colors = [UIColor.GradientColors.midnightBlackLight.cgColor, UIColor.GradientColors.midnightBlackLight.cgColor]
			 locations = [0.0, 1.0]  // 처음부터 끝까지 동일한 흰색
		 }
 ...
 // MARK: - UIView Extension for Easy Gradient Application
 extension UIView {
 ...
 extension UIViewController {
	 ...
	 func setupGradientTraitObserver() {
		 // iOS 13+ 에서만 다크모드 변경 감지 가능
		 if #available(iOS 13.0, *) {
			 // 이미 traitCollectionDidChange를 override 하고 있다면
			 // handleGradientTraitCollectionChange()를 호출하면 됨
		 }
	 }
	 
	 // 이 메서드를 ViewController의 traitCollectionDidChange에서 호출하세요
	 func handleGradientTraitCollectionChange() {
		 updateGradientsInView(view)
	 }
	 
	 // 더 안전한 업데이트 방법 (그라디언트 타입을 알고 있을 때)
	 func reapplyGradientForTraitCollectionChange(_ gradientType: CAGradientLayer.GradientType) {
		 view.reapplyGradientForTraitCollection(gradientType)
	 }
