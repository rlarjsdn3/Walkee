...
final class HealthDataViewModel: ObservableObject {
    ...
    func getWeeklyHealthData() async -> WeeklyHealthData {
        ...
        let daysFromMonday = (weekday == 1) ? 6 : weekday - 2   // 이번주 월요일 찾기, 일요일이면 6일 뒤로
    ...
    func getMonthlyHealthData() async -> MonthlyHealthData {
        ...
        let thisMonthEnd = calendar.date(byAdding: .second, value: -1, to: calendar.dateInterval(of: .month, for: today)!.end)!
        ...
        let lastMonthEnd = calendar.date(byAdding: .second, value: -1, to: calendar.dateInterval(of: .month, for: lastMonthDate)!.end)!
